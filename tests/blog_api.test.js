...
describe('testing blog functionality', () => {
  ...
  test('blog is deleted successfully', async () => {
    ...
    const blogToDelete =
      blogsBeforeDelete.body[blogsBeforeDelete.body.length - 1] // viimeinen eli äsken lisätty blogi
    ...
    expect(blogsAfterDelete.body).toHaveLength(
  ...
  test('blog is updated successfully', async () => {
    ...
    await api.put(`/api/blogs/${allBlogs.body[0].id}`).send(newBlog).expect(200)
  })
})
